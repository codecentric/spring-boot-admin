...
| `spring.boot.admin.ssrf-protection.allowed-cidrs`Additional IP addresses or CIDR ranges (IPv4 and IPv6) that are permitted even if they would otherwise be blocked by the default {@code InetAddressFilter.externalAddresses()} filter. Useful for intranet deployments where the Admin Server must reach services on private IP ranges.Examples: {@code 192.168.1.100}, {@code 192.168.1.0/24}, {@code 10.0.0.0/8}, {@code fd00::/8}- Type: 
