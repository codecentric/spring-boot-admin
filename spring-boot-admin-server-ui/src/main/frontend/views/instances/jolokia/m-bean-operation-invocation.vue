  ...
  methods: {
    ...
    async execute() {
      ...
      try {
        ...
        if (!this.resultContainsErrorStatus(result)) {
          this.result = this.parseValue(result.data);
          this.state = 'completed';
        } else {
          const parsedResult = this.parseValue(result.data);
    ...
    resultContainsErrorStatus(result) {
      if (result.status >= 400) {
        return true;
      }

      const parsedResponse = this.parseValue(result.data);
