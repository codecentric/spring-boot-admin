    ...
    <template #default>
      ...
      <div class="-mx-4 -my-3">
        ...
        <template v-for="healthGroup in healthGroups">
          <div class="px-4 py-2 border-t sm:px-6" :class="{'border-b': isHealthGroupOpen(healthGroup.name)}">
            <h4 class="leading-6 font-medium text-gray-900">
              <button @click="toggleHealthGroup(healthGroup.name)" class="flex items-center" :aria-label="$t('instances.details.health_group.title') + ': ' + healthGroup.name">
                <font-awesome-icon v-if="isHealthGroupCollapsible(healthGroup.name)" icon="chevron-down"
                                   class="transition-[transform] mr-2 h-4"
                                   :class="{'-rotate-90': !isHealthGroupOpen(healthGroup.name)}"/>
  ...
  methods: {
    ...
    toggleHealthGroup(groupName: string) {
      if (this.isHealthGroupCollapsible(groupName)) {
        this.healthGroupOpenStatus[groupName].isOpen = !this.healthGroupOpenStatus[groupName].isOpen;
      }
    },
    async fetchHealth() {
      this.error = null;
      ...
      try {
        ...
        if (Array.isArray(res.data.groups)) {
          ...
          this.healthGroupOpenStatus = this.healthGroups
              .map((group: { name: string, data: any }) => {
                return ({
                  [group.name]: {
