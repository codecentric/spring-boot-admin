...
describe('HttpExchanges - excludeActuator', () => {
  ...
  describe('when actuator path is not present', () => {
    it('does not show exclude actuator checkbox', async () => {
      const fetchHttpExchanges = vi.fn().mockResolvedValue({
        data: {
          exchanges: [],
        },
      ...
      // The checkbox should not be present when managementUrl is on a different host
