...
==== DingTalk Notifications ====
To enable https://www.dingtalk.com/[Telegram] notifications you need to create and authorize a dingtalk bot and set the appropriate configuration properties for accessToken and secret.
...
| spring.boot.admin.notify.dingtalk.webhookUrl
| The DingTalk webhook url to send the notifications to.
|

| spring.boot.admin.notify.dingtalk.accessToken
| The token to access the DingTalk API.
|
