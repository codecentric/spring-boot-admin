...
All Notifiers are sharing a common `RestTemplate` which can be configured to use a proxy.
...
If you need proxy authentication, you need to configure your own `RestTemplate`.
[source,java]
----
@Bean
...
You need to add an additional dependency to use the authentication features of Apache HttpClient.
[source,xml]
.pom.xml
----
<dependency>
