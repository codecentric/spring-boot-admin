...
The JavaScript-Bundle and CSS-Stylesheet must be placed on the classpath at `/META-INF/spring-boot-admin-server-ui/assets/extensions/{name}/` so the server can pick them up.
...
If new top level routes are added to the frontend, they also must be known to the backend. Add a `/META-INF/spring-boot-admin-server-ui/assets/extensions/{name}/routes.txt` with all your new toplevel routes (one route per line).
