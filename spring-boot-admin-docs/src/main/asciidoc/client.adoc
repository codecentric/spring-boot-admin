...
| Select which information should be considered when sending the URL of a service: +
    * `IP`: Uses the IP returned by `InetAddress.getHostAddress()` +
    * `HOST_NAME`: Uses the hostname of a single machine returned by `InetAddress.getHostName()` +
