...
Instead of the declarative approach for setting admin client's configuration options, you could register your
custom ApplicationFactory.

[source,java]
.MyApplicationFactory.java
----
public class MyApplicationFactory extends ApplicationFactory {

    @Override
    protected String doServiceHost() {
        String host = super.doServiceHost();
        //replace the "public" hostname with the "private" one, e.g.: XXX.dmesg.de ==> XXXi.dmesg.de
        return !host.contains("i.dmesg.de") ? host.replace(".dmesg.de", "i.dmesg.de") : host;
    }
}
