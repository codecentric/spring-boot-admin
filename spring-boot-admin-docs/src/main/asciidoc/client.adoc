...
| spring.boot.admin.client.instance.service-url-type
| Select which information should be considered when guessing the URL of a service: +
    * `IP`: Uses the IP returned by `InetAddress.getHostAddress()` +
    * `HOST_NAME`: Uses the value returned by `InetAddress.getHostName()` +
    * `CANONICAL_HOST_NAME`: Uses the value returned by `InetAddress.geCanonicalHostName()` +
