...
| spring.boot.admin.client.metrics
| List of metrics to display on the metrics page. The values should be a comma separated list e.g. `spring.boot.admin.client.metrics: threads, web.access, jvm.memory, datasource.primary, counter, gauge`.
| Will use `counter, gauge` as default when no values provided
