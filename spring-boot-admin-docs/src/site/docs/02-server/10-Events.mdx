...
### InstanceStatusDetailsChangedEvent
Occurs when the health details of an instance change but the top-level status code (e.g., `UP`, `DOWN`) remains the same.
For example, disk space metrics, database connection counts, or custom health indicator details that fluctuate between polls will emit this event instead of an `InstanceStatusChangedEvent`.
This finer-grained event allows the UI to refresh health details without triggering a full status-change notification.
