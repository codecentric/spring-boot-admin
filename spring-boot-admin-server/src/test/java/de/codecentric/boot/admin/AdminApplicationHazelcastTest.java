...
public class AdminApplicationHazelcastTest {
  ...
  public void setup() throws InterruptedException {
    ...
    instance1 = (EmbeddedWebApplicationContext) SpringApplication.run(TestAdminApplication.class,
            "--server.port=0", "--spring.jmx.enabled=false");
    instance2 = (EmbeddedWebApplicationContext) SpringApplication.run(TestAdminApplication.class,
            "--server.port=0", "--spring.jmx.enabled=false");
  }
  ...
  private ResponseEntity<Map<String, String>> getApp(String id, EmbeddedWebApplicationContext context) {
    int port = context.getEmbeddedServletContainer().getPort();
    @SuppressWarnings("unchecked")
    ResponseEntity<Map<String, String>> response = template.getForEntity(
        "http://localhost:" + port + "/api/applications/" + id, (Class<Map<String, String>>) (Class<?>) Map.class);
    return response;
  }
  ...
  private ResponseEntity<Map<String, String>> registerApp(String name, String healthUrl,
    ...
    ResponseEntity<Map<String, String>> responseEntity = template.postForEntity(
        "http://localhost:" + port + "/api/applications", app, (Class<Map<String, String>>) (Class<?>) Map.class);
  ...
  public static class TestAdminApplication {
    ...
    public Config hazelcastConfig() {
      ...
      config.addListConfig(new ListConfig("spring-boot-admin-application-store").setBackupCount(1).setMaxSize(1000));
