...
public class RemindingNotifierDiffblueTest {
   ...
   * </ul>
   * <p>
   * Method under test: {@link RemindingNotifier#RemindingNotifier(Notifier, InstanceRepository)}
   */
  @Test
  ...
  public void testSendReminders() throws AssertionError {
    ...
    FirstStep<Void> createResult = StepVerifier.create(
        new RemindingNotifier(delegate, new EventsourcingInstanceRepository(new InMemoryEventStore())).sendReminders());
    createResult.expectComplete().verify();
  }
  ...
  public void testShouldEndReminder_thenReturnFalse() {
    ...
    // Act and Assert
    assertFalse(
        remindingNotifier.shouldEndReminder(new InstanceEndpointsDetectedEvent(instance, 1L, Endpoints.empty())));
  }
