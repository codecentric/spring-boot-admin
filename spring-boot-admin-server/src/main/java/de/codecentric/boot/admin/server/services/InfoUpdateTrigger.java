...
public class InfoUpdateTrigger extends ResubscribingEventHandler<ClientApplicationStatusChangedEvent> {
    ...
    protected Publisher<?> handle(Flux<ClientApplicationStatusChangedEvent> publisher) {
        return publisher.subscribeOn(Schedulers.newSingle("info-updater")).flatMap(this::updateInfo);
    }

    protected Mono<Void> updateInfo(ClientApplicationStatusChangedEvent event) {
